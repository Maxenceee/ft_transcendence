...
class websocket_client(WebsocketConsumer):
	...
	def wallCollideTwoPlayer(self):
		if self.data['ball']['x'] < -19 :
			self.data['ballDirection']['x'] *= -1
		elif self.data['ball']['x'] > 19:
			self.data['ballDirection']['x'] *= -1
		if self.data['ball']['z'] < -31:
			self.data['ball']['x'] = 0
			self.data['ball']['z'] = 0 
			self.data['ball']['y'] = 0	
	...
	def receive(self, text_data=None, bytes_data=None):
		...
		self.data['ball']['x'] += ( self.data['ballDirection']['x'] ) * 0.1 * self.data['moveSpeed'] 
		self.data['ball']['z'] += ( self.data['ballDirection']['z'] ) * 0.1 * self.data['moveSpeed']  
		# logging.info("after")
		# logging.info(self.data['ball']['x'])
